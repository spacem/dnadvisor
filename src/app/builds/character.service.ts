...
export class CharacterService {
  ...
  getHeroStats(heroLevel: any) {
    let heroStats = [];
    if (heroLevel > 0) {
      const index = this.dntService.findFast(heroLevels, 'id', heroLevel);
      if (index.length === 1) {
        const h = this.dntService.getRow(heroLevels, index[0]);
        if (h) {
          const pIndex = this.dntService.findFast(heroLevelPotentials, 'PotentialID', h.HeroLevelAbilityID);
          if (pIndex.length === 1) {
            const p = this.dntService.getRow(heroLevelPotentials, pIndex[0]);
            if (p) {
